...
class CATS_Attention(nn.Module):
    ...
    def forward(self, X):
        ...
        self.Xp1score = self.Xp1valid * (torch.mm(self.va, self.tanh(torch.mm(self.Wa, torch.cat((self.Xq, self.Xp1), 1)))))
        self.Xp2score = self.Xp2valid * (torch.mm(self.va, self.tanh(torch.mm(self.Wa, torch.cat((self.Xq, self.Xp2), 1)))))
