...
def run_model(qry_attn_file_train, qry_attn_file_test, train_pids_file, test_pids_file, train_pvecs_file,
    ...
    #opt = optim.Adadelta(m.parameters(), lr=lrate)
    ...
    for i in range(epochs):
        ...
        for b in range(math.ceil(train_samples//batch)):
            ...
            loss = loss_func(ypred, y_train_curr)
            ...
            if b % 100 == 0:
                m.eval()
                ypred_val = m(X_val)
                val_loss = loss_func(ypred_val, y_val)
                val_auc = roc_auc_score(y_val.detach().cpu().numpy(), ypred_val.detach().cpu().numpy())

                ypred_test = m(X_test)
                test_loss = loss_func(ypred_test, y_test)
                test_auc = roc_auc_score(y_test.detach().cpu().numpy(), ypred_test.detach().cpu().numpy())
                print(
                    '\rTrain loss: %.5f, Train auc: %.5f, Val loss: %.5f, Val auc: %.5f, Test loss: %.5f, Test auc: %.5f' %
...
def main():
    ...
    parser.add_argument('-trp', '--train_pids', default="y1train-all-pids.npy")
    parser.add_argument('-trv', '--train_pvecs', default="y1train-all-paravecs.npy")
    parser.add_argument('-trq', '--train_qids', default="half-y1train-qry-attn-context-leadpara-qids.npy")
    parser.add_argument('-trqv', '--train_qvecs', default="half-y1train-qry-attn-context-leadpara-qvecs.npy")
    ...
    parser.add_argument('-bt', '--batch', type=int, default=256)
